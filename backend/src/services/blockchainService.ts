...
const DOCUMENT_VERIFICATION_ABI = [
  ...
  "function institutions(address) external view returns (string memory name, string memory registrationNumber, string memory contactInfo, bool isVerified, uint256 registrationDate)",
  "function institutionAddresses(uint256) external view returns (address)",
  "function owner() external view returns (address)",
  "event DocumentIssued(bytes32 indexed documentHash, address indexed issuer, string recipientName, string documentType, string title, uint256 issuanceDate)",
  "event DocumentVerified(bytes32 indexed documentHash, address indexed verifier, uint256 verificationDate)", // <-- This event has the verifier
  "event DocumentRevoked(bytes32 indexed documentHash, address indexed issuer, uint256 revocationDate)",
  "event InstitutionRegistered(address indexed institutionAddress, string name, uint256 registrationDate)",
  "event InstitutionVerified(address indexed institutionAddress, bool verified)"
];
...
class BlockchainService {
  ...
  // ... (isInstitutionVerified, register, etc. remain unchanged) ...
  public async isInstitutionVerified(address: string): Promise<boolean> {
    try {
      const isVerified = await this.contract.isInstitutionVerified(address);
      return isVerified;
  ...
  public async getDocumentStats(issuerAddress: string): Promise<DocumentStats> {
    try {
      ...
      // --- MODIFICATION: Use a Map to store verifier address ---
      const verifiedMap = new Map<string, string>();
      for (const event of allVerifiedEvents) {
        const hash = event.args?.documentHash;
        const verifier = event.args?.verifier;
        // Only add if it's a hash we care about (issued by this institution)
        if (hash && verifier && issuedHashes.has(hash) && !verifiedMap.has(hash)) {
          verifiedMap.set(hash, verifier);
        }
      }
      const verifiedDocuments = verifiedMap.size;
      // --- END MODIFICATION ---
      ...
      let pendingCount = 0;
      for (const hash of issuedHashes) {
        if (!verifiedMap.has(hash) && !revokedHashes.has(hash)) {
          pendingCount++;
        }
      }
    ...
    try {
      ...
      for (const event of paginatedEvents) {
        try {
          ...
          const isRevoked = revokedHashes.has(documentHash);
          const isVerified = verifiedMap.has(documentHash); // <-- Use Map
          ...
          documents.push({
            ...
            isActive: doc.isActive && !isRevoked && !isExpired,
            isVerified: isVerified,
            verifiedBy: verifiedMap.get(documentHash) || null, // <-- ADDED
            isRevoked,
            transactionHash: event.transactionHash,
            blockNumber: event.blockNumber
          });
  ...
  // ... (getDocument, lookupAddress, verifyInstitution, getInstitutionDetails, getAllInstitutionsFromChain remain unchanged) ...
  ...
  public async getDocument(documentHash: string): Promise<DocumentDetails | null> {
    try {
      ...
      // --- MODIFICATION: Fetch verifier address ---
      let verifiedBy: string | null = null;
      if (isVerified) {
        const verifyFilter = this.contract.filters.DocumentVerified(documentHash);
        const verifyEvents = await this.contract.queryFilter(verifyFilter, 0, 'latest') as EventLog[];
        if (verifyEvents.length > 0) {
          verifiedBy = verifyEvents[0].args?.verifier || null;
        }
      }
      // --- END MODIFICATION ---
  ...
  // --- MODIFICATION: Updated getAllDocuments ---
  public async getAllDocuments(
    limit: number = 10, 
    offset: number = 0
  ): Promise<DocumentDetails[]> {
    try {
      ...
      const issueFilter = this.contract.filters.DocumentIssued();
      const issuedEvents = await this.contract.queryFilter(issueFilter, 0, 'latest') as EventLog[];
      ...
      for (const event of allVerifiedEvents) {
        const hash = event.args?.documentHash;
        const verifier = event.args?.verifier;
        if (hash && verifier && allIssuedHashes.has(hash) && !verifiedMap.has(hash)) {
          verifiedMap.set(hash, verifier);
        }
      }
      // --- END MODIFICATION ---
