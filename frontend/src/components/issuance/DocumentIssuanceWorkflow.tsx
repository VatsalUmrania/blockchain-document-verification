...
import { DocumentMetadata, DocumentMetadataInput } from '../../types/document.types';
...
const DocumentIssuanceWorkflow: React.FC = () => {
  ...
  const [metadata, setMetadata] = useState<DocumentMetadata>(new DocumentMetadata({ documentType: '', recipientName: '' }));
  ...
  const toastShownRef = useRef(false); // <-- FIX: Add ref to track toast
  ...
  useEffect(() => {
    const checkInstitutionStatus = async (): Promise<void> => {
      ...
      // --- FIX: Prevent double run in StrictMode ---
      if (toastShownRef.current && !checkingStatus) {
        setCheckingStatus(false);
        return; 
      }
      // --- End Fix ---
        ...
        if (isVerified) {
          ...
          if (!toastShownRef.current) {
            toast.success('Institution Verified', {
              description: 'You can now issue documents.',
            });
            toastShownRef.current = true; // <-- Set the ref
          }
        } else {
          // --- FIX: Check ref ---
          if (!toastShownRef.current) {
            toast.info('Registration Required', {
              description: 'Please register your institution to issue documents.',
            });
            toastShownRef.current = true; // <-- Set the ref
          }
        }
  ...
  }, [isConnected, signer, provider, checkingStatus]); // Added checkingStatus to dependency array
  ...
  const registerInstitution = useCallback(async (): Promise<void> => {
    ...
    const { isValid, errors } = metadata.validateInstitution();
    if (!isValid) {
      toast.error('Missing Information', {
        description: `Please fill in required fields: ${errors.join(', ')}`,
      });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        toast.success('Institution Registered', {
          description: 'Your institution was registered successfully!',
        });
        toast.info('Verification Pending', {
          description: 'An admin must verify your institution before you can issue documents.',
        });
        setCurrentStep(2);
      }
    } catch (error) {
  ...
  const verifyInstitution = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('Institution Verified', {
          description: `Address ${institutionAddress.slice(0, 6)}...${institutionAddress.slice(-4)} is now verified!`,
        });
  ...
  // ← UPDATED: Issue document on blockchain with title
  const issueDocument = useCallback(async (): Promise<void> => {
    if (!selectedFile || !isConnected || !provider || !signer) {
    ...
    if (!validation.isValid) {
      toast.error('Validation Failed', {
        description: `Please fill all required fields: ${validation.errors.join(', ')}`,
      });
      return;
    }
    ...
    try {
      ...
      const documentHash = blockchainService.createDocumentHash(fileContent);
      
      console.log('📝 Issuing document with:', {
        documentHash,
      ...
      // ← UPDATED: Pass title parameter
      const result = await blockchainService.issueDocument(
        documentHash, 
        metadata, 
        metadata.title,  // ← ADD THIS - Pass the title
        '', 
        '0x'
      );
      ...
      if (result.success) {
        ...
        toast.success('Document Issued', {
          description: 'Document has been successfully issued on the blockchain!',
        });
        setCurrentStep(4);
      }
    } catch (error) {
  ...
  const resetWorkflow = useCallback((): void => {
    ...
    setMetadata(new DocumentMetadata({ documentType: '', recipientName: '' }));
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          >
            ...
            {/* [MODIFIED] Removed gradient text for a cleaner look */}
        ...
        {/* [MODIFIED] Connection Status - changed to destructive variant */}
        {!isConnected && (
          <motion.div
          ...
          >
            <Alert variant="destructive" className="mb-6">
              ...
              <AlertDescription className="flex w-full items-center justify-between">
                ...
                <Button onClick={connectWallet} size="sm" variant="destructive">
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  {/* [MODIFIED] Institution Status - replaced green with primary */}
                  {!checkingStatus && institutionVerified && (
                    <Alert className="border-primary/20 bg-primary/10">
                      <CheckCircle className="h-4 w-4 text-primary" />
                      <AlertDescription>
                        <div className="space-y-2">
                          <h3 className="text-primary font-medium">Institution Verified</h3>
                          <p className="text-sm text-primary/80">Your institution is registered and verified. You can issue documents!</p>
                  ...
                  {!checkingStatus && !institutionVerified && (
                    <div className="space-y-6">
                      ...
                      <Alert>
                        ...
                        <AlertDescription>
                          <div className="space-y-2">
                            ...
                            <p className="text-sm">
                              After registration, your institution must be verified by an Admin before you can issue documents.
                            </p>
                            <div className="text-xs space-y-1 mt-3">
                              <p>• <strong>Admin Wallet:</strong> 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266</p>
                              ...
                              <p>• Only a designated Admin can verify institutions</p>
                              <p>• Switch to the Admin account and click "Verify Institution"</p>
                            </div>
                          </div>
                        </AlertDescription>
                      </Alert>
                      ...
                      <div className="flex justify-end">
                        ...
                        >
                          {isProcessing ? (
                            <Loader2 className="w-4 h-4 mr-2 animate-spin tex" />
                          ) : null}
                          Register Institution
                        </Button>
                      </div>
          ...
          {/* [MODIFIED] Step 2: Document Upload (using theme colors) */}
          {currentStep === 2 && (
            <motion.div
              key="step2"
              initial={{ opacity: 0, x: 20 }}
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      isDragActive
                        ...
                        : selectedFile
                          ? "border-primary/50 bg-primary/10" // Replaced green
          ...
          {/* [MODIFIED] Step 3: Metadata & Issue (using theme colors) */}
          {currentStep === 3 && (
            <motion.div
              key="step3"
              initial={{ opacity: 0, x: 20 }}
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      <Label htmlFor="issuerNameDisplay">Issuing Institution</Label>
                      <Input
                        id="issuerNameDisplay"
                        type="text"
                        readOnly
                        value={metadata.issuerName || ''}
                        className="border-dashed bg-muted"
                        placeholder="Institution name not found on blockchain!"
                      />
                      {!metadata.issuerName && (
                        <p className="text-destructive text-sm mt-1">
                          The issuer name is missing. The validation will fail.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateMetadata('expirationDate', e.target.value ? new Date(e.target.value) : undefined)}
                      />
                    </div>
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) => updateMetadata('graduationDate', e.target.value ? new Date(e.target.value) : undefined)}
                          />
                        </div>
                      </>
                    )}
          ...
          {/* [MODIFIED] Step 4: QR Code & Complete (using theme colors) */}
                ...
                <AlertDescription>
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="font-medium text-primary">Transaction Hash:</span>
                      ...
                      <div>
                        <span className="font-medium text-primary">Document Hash:</span>
                        <div className="font-mono text-muted-foreground break-all">
                          {issuanceResult.documentHash}
                        </div>
                      </div>
                      <div>
                        <span className="font-medium text-primary">Gas Used:</span>
